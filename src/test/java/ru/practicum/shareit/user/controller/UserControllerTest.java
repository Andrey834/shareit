...
class UserControllerTest {
    ...
    void create_whenInvoked_thenResponseStatusOkWithUserInBody() {
        ...
        Mockito.when(userService.create(new UserDto())).thenReturn(expectedUserDto);
    ...
    void update_whenInvoked_thenResponseStatusOkWithUserInBody() {
        int userId = 0;
        UserDto expectedUserDto = new UserDto();
        Mockito.when(userService.update(userId, new UserDto())).thenReturn(expectedUserDto);
    ...
    void get() {
        int userId = 0;
        UserDto expectedUserDto = new UserDto();
        Mockito.when(userService.get(userId)).thenReturn(expectedUserDto);
    ...
    void getAll_whenInvoked_thenResponseStatusOkWithUsersCollectionInBody() {
        ...
        Mockito.when(userService.getAll()).thenReturn(expectedUsersDto);
    ...
    void delete_whenInvoked_thenResponseStatusOkWithBooleanTrue() {
        ...
        Mockito.when(userService.delete(userId)).thenReturn(expectedBoolean);
